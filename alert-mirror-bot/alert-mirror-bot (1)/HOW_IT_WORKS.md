...
3. **Embed mode (if on).** If you typed your configured ping role at the start (e.g. `@ShowTime Alerts`), it's stripped from the alert text so the title reads clean. The ping is resent separately as plain content so it still notifies. The first line becomes a bold title (if there's more text after it), and the disclaimer URL (if set) is appended to the bottom.
4. **Split if long.** Discord caps messages at 2000 characters. Long alerts get cut at line breaks, never mid-sentence or mid-URL. In embed mode, embeds are split so each message stays under Discord's per-message aggregate limit.
5. **Find the channel per server.** Missing channel or no permission = that server's skipped, doesn't affect anyone else.
6. **Handle images.** Downloaded once, re-uploaded as real files everywhere. Discord's raw attachment links expire after ~24h — a lot of cheap mirror bots just copy the link and the image quietly dies the next day. This doesn't. Oversized/broken downloads fall back to the original link instead of dropping the image.
7. **Send, isolated per server.** One server erroring out gets logged and skipped; everyone else still gets their copy.
8. **Order.** Back-to-back alerts stay in the order you posted them (a lock serializes it).
