...
def test_embed_strip_typed_role_mention_from_first_line():
    src, g = Guild(name="src"), Guild([Chan()], roles=[Role("vip")])
    run(Msg("vip BTO NVDA 145C\nentry $2.10", guild=src), [src, g], ping="vip", use_embeds=True)
    embed = embeds_of(g)[0]
    assert embed.title == "BTO NVDA 145C"
    assert "vip" not in embed.title


def test_embed_disclaimer_url_appended_to_last_embed():
    src, g = Guild(name="src"), Guild([Chan()])
    run(Msg("t\nbody", guild=src), [src, g], use_embeds=True,
        disclaimer_url="https://docs.google.com/example")
    embed = embeds_of(g)[0]
    assert "https://docs.google.com/example" in embed.description
